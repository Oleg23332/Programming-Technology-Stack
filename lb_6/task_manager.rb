...
class TaskManager
  ...
  def edit_task(title, new_title: nil, new_deadline: nil, new_status: nil)
    ...
    if task
      ...
      # Перевіряємо, чи new_deadline має правильний формат дати
      if new_deadline && !new_deadline.empty?
        begin
...
class TaskApp
  ...
  def edit_task
    ...
    # Формуємо хеш тільки з непорожніми значеннями
    update_params = {}
    update_params[:new_title] = new_title unless new_title.empty?
    update_params[:new_deadline] = new_deadline unless new_deadline.empty?
    update_params[:new_status] = new_status unless new_status.empty?

    @task_manager.edit_task(title, **update_params)
  ...
  def filter_tasks
    ...
    # Формуємо хеш лише з непорожніми значеннями
    filter_params = {}
    filter_params[:status] = status unless status.empty?
    filter_params[:due_date] = due_date unless due_date.empty?

    tasks = @task_manager.filter_tasks(**filter_params)
    display_tasks(tasks)
  end
